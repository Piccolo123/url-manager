...
当用户要把 X/Twitter 上的资料整理进 AI 足迹时，先用 Hermes Tweet 做社交来源发现：

1. 用 `tweet_explore` 找到可用的 Xquik API catalog endpoint。
2. 用 `tweet_read` 搜索推文、读取回复、查看用户资料或拉取监控结果。
3. 将有价值的公开 URL、标题、摘要和标签整理后，再交给 `ai-footprints`
   skill 调用 `POST /collections`。

不要把 `XQUIK_API_KEY`、`FOOTPRINTS_TOKEN` 或其他凭据写入足迹内容、标签、
摘要、聊天记录或工具参数。
