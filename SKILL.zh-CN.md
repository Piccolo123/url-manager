...
| "改个标题/移到另一个分类" | `python3 {baseDir}/scripts/footprints.py update <id> --title <t> --category-ids <ids> [--json]` |
| "帮我把收藏整理一下" | `python3 {baseDir}/scripts/footprints.py batch-update '<json>' [--json]` |
| "新建一个分类" | `python3 {baseDir}/scripts/footprints.py create-category <名称> [--json]` |
| "建个工作区" | `python3 {baseDir}/scripts/footprints.py create-category-set <名称> [--json]` |
| "建一个共享收藏夹" | `python3 {baseDir}/scripts/footprints.py create-shared-category <名称> --mode cocreate [--json]` |
| "把邀请链接发给同事" | `python3 {baseDir}/scripts/footprints.py create-invite-link <sc_id> [--json]` |
| "我有个邀请码" | `python3 {baseDir}/scripts/footprints.py join-shared-category <code> [--json]` |
| "把这个加到团队收藏里" | `python3 {baseDir}/scripts/footprints.py add-to-shared <sc_id> --collection-id <id> [--json]` |
...
| 命令 | 用户说……时使用 |
|------|--------------|
| `python3 {baseDir}/scripts/footprints.py update <id> --title <t> --description <d> --category-ids <ids> --tags <tags>` | "改个标题 / 移到另一个分类" |
| `python3 {baseDir}/scripts/footprints.py batch-update <updates>` | "帮我把收藏整理一下"（最多 50 条） |
| `python3 {baseDir}/scripts/footprints.py categories` | 了解有哪些分类 |
| `python3 {baseDir}/scripts/footprints.py create-category <name> [--category-set-id <id>]` | "新建一个分类" |
| `python3 {baseDir}/scripts/footprints.py tags` | 了解已有标签 |
| `python3 {baseDir}/scripts/footprints.py category-sets` | 列出分类集 |
| `python3 {baseDir}/scripts/footprints.py create-category-set <name>` | "建个工作区" |
...
5. python3 {baseDir}/scripts/footprints.py update id category_ids="..." → 归类
...
```
1. python3 {baseDir}/scripts/footprints.py me → 确认身份
2. python3 {baseDir}/scripts/footprints.py categories + python3 {baseDir}/scripts/footprints.py tags → 了解当前结构
3. python3 {baseDir}/scripts/footprints.py search query → 找到目标
4. python3 {baseDir}/scripts/footprints.py add / python3 {baseDir}/scripts/footprints.py update → 操作
```
...
```
1. python3 {baseDir}/scripts/footprints.py list --limit 100 → 获取全部收藏
2. python3 {baseDir}/scripts/footprints.py categories → 规划目标分类
...
```bash
python3 {baseDir}/scripts/footprints.py create-category "新主题"    # → 返回新分类 ID
python3 {baseDir}/scripts/footprints.py list --limit 100
# 对每一条匹配的收藏：
python3 {baseDir}/scripts/footprints.py update <id> --category-ids <现有ID>,<新分类ID>
```

### 合并两个分类

```bash
python3 {baseDir}/scripts/footprints.py categories                      # 记下源和目标 ID
python3 {baseDir}/scripts/footprints.py list --category-id <源分类ID>   # 列出源分类下全部收藏
# 逐条将源分类 ID 替换为目标分类 ID
python3 {baseDir}/scripts/footprints.py update <id> --category-ids <目标ID>,<其他ID>
...
# 逐条追加：
python3 {baseDir}/scripts/footprints.py update <id> --category-ids <现有ID>,<GitHub分类ID>
```
...
# 逐条追加目标分类：
python3 {baseDir}/scripts/footprints.py update <id> --category-ids <现有ID>,<目标分类ID>
```
...
```bash
python3 {baseDir}/scripts/footprints.py tags        # 高频标签
python3 {baseDir}/scripts/footprints.py categories  # 已有分类
...
```bash
python3 {baseDir}/scripts/footprints.py categories  # 找到目标个人分类 ID
python3 {baseDir}/scripts/footprints.py copy <收藏ID> --category-ids <个人分类ID>
```
...
```
1. Agent A：python3 {baseDir}/scripts/footprints.py create-shared-category "团队知识库" --mode cocreate
2. Agent A：python3 {baseDir}/scripts/footprints.py create-invite-link <sc_id> → 把邀请码发给用户
3. 用户转发给同事
4. Agent B：python3 {baseDir}/scripts/footprints.py join-shared-category <code>
5. 双方都能通过 python3 {baseDir}/scripts/footprints.py search 看到彼此的添加
```
...
# ✅ 正确：先查现有分类，再合并
python3 {baseDir}/scripts/footprints.py get 42  # → 现有分类 [3, 5]
...
每次调用都会创建全新空白账号。务必先检查是否已有保存的 Token。不确定时先调 `python3 {baseDir}/scripts/footprints.py me` 确认。
...
### 始终做到
- **静默自动注册** — 不要为账号设置中断用户
- **先搜索再列举** — 用 `python3 {baseDir}/scripts/footprints.py search` 精准查询，而非全量导出
- **先了解再创建** — 添加前先调 `python3 {baseDir}/scripts/footprints.py categories` 和 `python3 {baseDir}/scripts/footprints.py tags` 避免重复
